...
Guideline chunks are independent of each other, so the chunk scan fans out
concurrently (bounded by a semaphore) instead of awaiting each chunk one at a
time — this is what keeps a multi-page guidelines document from turning into
a multi-second sequential bottleneck.
"""
...
from agents.llm_utils import call_json_model, safe_score
...
# Bounds how many guideline chunks are scanned concurrently. Keeps latency
# low without bursting past typical per-key rate limits.
...
{{
  ...
  "search_strategy_used": "string"
        ...
        result = await self._call(prompt, fallback, max_tokens=1400)
    ...
    async def _search_brand_requirements(
        ...
        # Independent chunks scan concurrently, bounded by the semaphore in
        # self._call, instead of one-at-a-time sequential awaits.
        chunk_results = await asyncio.gather(
            *[
                self._scan_chunk(chunk, advisor_context, search_hints, flexibility_indicators)
                for chunk in chunks[:_MAX_CHUNKS]
            ]
        )
        all_requirements: List[dict] = [req for chunk_reqs in chunk_results for req in chunk_reqs]
        ...
        deduped = sorted(
            deduped,
            key=lambda r: safe_score(r.get("relevance_score", 0)),
            reverse=True,
        )[:12]
...
{{
  ...
  "recommendation": "string"
        ...
        result = await self._call(prompt, fallback, max_tokens=1600)
