...
class AdvisorAgent:
    ...
    async def analyze_ehr(self, creator_profile: CreatorProfile) -> dict:
        ...
        audience_analysis = await self._analyze_audience(creator_profile)
        metrics_analysis = await self._analyze_metrics(creator_profile)
    ...
    def _safe_json_parse(self, text: Any) -> dict:
        ...
        try:
            ...
            return {"error": "Failed to parse JSON", "parse_error": str(e), "raw_text": text}

    async def _call_json_model(self, prompt: str, fallback: dict, max_tokens: int = 1200) -> dict:
        try:
...
{{
  ...
  "summary": "string"
        ...
        result = await self._call_json_model(prompt, fallback, max_tokens=800)
...
{{
  ...
  "performance_summary": "string"
        ...
        result = await self._call_json_model(prompt, fallback, max_tokens=600)
...
{{
  ...
  "key_risks": ["string"]
        ...
        result = await self._call_json_model(prompt, fallback, max_tokens=1000)
